...
2. **If `graph_continue` returns `skip=true`**: fewer than 5 files â€” read only specifically named files.

3. **Read `recommended_files`** using `graph_read`.
   - Always use `file::symbol` notation (e.g. `src/auth.ts::handleLogin`) â€” never read whole files.
...
Use this state â€” not prose summaries â€” to remember what's been done across turns.
...
- `max_supplementary_greps` and `max_supplementary_files` are hard caps â€” never exceed them.
- Do NOT call `graph_continue` more than once per turn.
- Always use `file::symbol` notation with `graph_read` â€” never bare filenames.
